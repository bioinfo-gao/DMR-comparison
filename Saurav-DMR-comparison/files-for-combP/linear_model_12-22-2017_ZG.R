...
#biocLite(lib="~/R/library")
# sudo apt-get install libmysqlclient-dev # run this line in terminla
#install.packages("RMySQL")
#biocLite(c("DMRcate"))
#biocLite("DMRcate",  dependencies=TRUE)
...
for (m in 8:8) {
    for (r in 1:length(seed.values))  {
        ...
        result <- runLinearModel(beta.values=temp,  cpg_number_cutoff = cpg_number_cutoffval, mval_each, r)
        ...
        write.csv(result.final, file=paste0("linear_model_result_for_mu", mval_each, "_repetition_", r,".csv"), row.names = FALSE)
    }
}
