...
result.all = as.data.frame(matrix(ncol=10, nrow=length(seed.values)*length(m.values)))##newly added

for (m in 1:length(m.values)) ##newly added
{
  print(paste0("m.values=",m.values[m]))
  ...
  for (r in 1:length(seed.values)) ##newly added
  {
    ...
    temp <- readRDS (paste0("simData_miu",m.values[m],"_rep",r,".rds") )
    ...
    simAclust.file <- read.csv (paste0("simAclust_for",m.values[m],"_rep",r,".csv"))
    ...
    write.table(result.all,file=paste("Combp_result.all_for_",length(m.values),"_number_of_mu_values_having_repitition_of_",length(seed.values),"_times.csv",sep=""),sep=",",row.names=F,col.names=T)##newly added
